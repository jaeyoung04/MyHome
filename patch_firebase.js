...
const targetPath = path.join('c:/Users/4-410/Downloads/MyHome-master', 'board.html');
let html = fs.readFileSync(targetPath, 'utf8');

// We know the script starts around `let currentOpenPostId = null;`
// We will replace `<script>\n        let currentOpenPostId = null;` to the EOF `</html>`

const scriptStartIndex = html.indexOf('<script>\n        let currentOpenPostId = null;');
if (scriptStartIndex === -1) {
    console.error("Could not find script start marker.");
    process.exit(1);
}

const beforeScript = html.substring(0, scriptStartIndex);
...
const newScript = `<script type="module">
        ...
        window.getMessages = async function() {
            ...
            try {
                ...
                querySnapshot.forEach((docSnap) => {
                    ...
                    // LocalStorage와 구조 맞추기
                    messages.push({
                        fs_id: docSnap.id,  // Firestore 실제 문서 ID (삭제/업데이트용)
                ...
                console.error("데이터를 가져오는 중 실패:", e);
                return [];
            }
        }
        ...
        window.renderBoard = async function() {
            ...
            if (filtered.length === 0) {
                ...
                tbody.innerHTML = \`<tr><td colspan="5" class="empty-row">\${emptyMsg}</td></tr>\`;
                return;
            }
        ...
        window.renderComments = async function(postId) {
            ...
            if(comments.length === 0) {
                commentList.innerHTML = '<div style="text-align: center; color: var(--text-secondary); font-size: 0.85rem; padding: 1rem;">아직 댓글이 없습니다. 첫 댓글을 남겨보세요!</div>';
                return;
            }
        ...
        window.updatePostInDB = async function(postId, updatedData) {
             ...
             if(!db) {
                 ...
                 messages[idx] = { ...messages[idx], ...updatedData };
                 localStorage.setItem('amk_board', JSON.stringify(messages));
             } else {
                 if(msg.fs_id) {
...
fs.writeFileSync(targetPath, beforeScript + newScript);
console.log("Successfully patched board.html with Firebase integration.");
